...
public class demo {
    ...
    public static void main(String[] args) {
        ...
        int []distance = new int[Recursive(point.length)];
        for(int i=0;i<point.length;i++){
            for(int j = i+1;j<point.length;j++){
                dis = (int)( Math.pow((point[j][0]-point[i][0]),2)+Math.pow((point[j][1]-point[i][1]),2));
                distance[num++]  = dis;
            }
        }
    ...
    public static final int Recursive(int n){
        if(0==n){
            return 0;
        }
        else
